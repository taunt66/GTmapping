...
def pileup_region_paired_to_file(args):
    ...
    try:
        ...
        logging.error(f"Error processing chunk {chrom}:{start}-{end}: {e}")
        # 确保临时文件被关闭和删除
        if tmpfh:
            tmpfh.close()
...
def get_genome_wide_pileup_paired(
    ...
    with Pool(processes=thread) as pool:
        ...
        for i, result in enumerate(results):
            ...
            # 每处理100个任务输出一次进度
            if (i + 1) % 100 == 0:
                logging.info(f"Completed {i + 1}/{len(tasks)} tasks")

    # 合并所有临时文件
    output_file = outputdir / f"{prefix}.pileup"
    logging.info(f"Merging {len(tmp_paths)} temporary files into {output_file}")
        ...
        with open(output_file, "w") as outfh:
            ...
            for i, tmp_path in enumerate(tmp_paths):
                if not os.path.exists(tmp_path):
                    logging.warning(f"Temporary file {tmp_path} does not exist, skipping")
                    continue
                ...
                try:
                    ...
                    logging.error(f"Error reading tmp file {tmp_path}: {e}")
                    continue
                finally:
                    # 删除临时文件
                    try:
                        ...
                        logging.warning(f"Could not remove tmp file {tmp_path}: {e}")
                
                # 进度日志
                if (i + 1) % 100 == 0:
                    logging.info(f"Merged {i + 1}/{len(tmp_paths)} temporary files")
    ...
    logging.info("Paired-end pileup writing completed.")
    return output_file
