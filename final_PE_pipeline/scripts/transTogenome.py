...
try:
    # 项目内导入，提供 BAM 排序和索引功能
    from .run_cmd_and_logging import sort_and_index_bam, get_logger
except ImportError:
    from run_cmd_and_logging import sort_and_index_bam, get_logger
...
def change_trans_to_genome(
    ...
    # 对输出的基因组 BAM 进行排序并建立索引，便于后续合并和分析
    logger = get_logger()
    sort_and_index_bam(outputbam, logger=logger, already_sorted=False)
