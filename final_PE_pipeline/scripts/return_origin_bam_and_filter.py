...
def restore_sequence(read: pysam.AlignedSegment, read_indices: dict) -> bool:
    ...
    返回:
        bool: 是否修改了序列
    """
    # 注意：在 pysam 中，直接给 read.query_sequence 赋值会将 query_qualities 置为 None
    # 因此在修改序列前需要保存原始质量值，修改后再显式恢复，否则质量信息会丢失。
    seq = list(read.query_sequence)
    original_quals = read.query_qualities  # 保存原始质量值（可能为 None）
    read_len = len(seq)
    ...
    if modified:
        # 更新序列，同时恢复原始质量值，避免 pysam 清空 query_qualities
        read.query_sequence = ''.join(seq)
        if original_quals is not None:
            read.query_qualities = original_quals
...
def filter_and_return_origin_bam(
    ...
    过滤逻辑：
    - 对于每个pair，检查所有target_base和cutoff条件
    - 只有当R1和R2在所有条件下都满足（即每个碱基的计数都小于对应的cutoff）时，才保留该pair
    """
    if logger is None:
    ...
    # 验证参数
    if len(target_bases) != len(cutoffs):
        raise ValueError(f"target_bases和cutoffs长度必须相同，"
                        f"got {len(target_bases)} and {len(cutoffs)}")
    
    # 验证target_bases
    for base in target_bases:
        if base.upper() not in ['A', 'C', 'G', 'T']:
            raise ValueError(f"target_base必须是A、C、G或T之一，当前值: {base}")
    
    # 转换为大写
    target_bases = [b.upper() for b in target_bases]
    
    logger.info(f"过滤条件: {len(target_bases)}个碱基类型")
    for base, cutoff in zip(target_bases, cutoffs):
        logger.info(f"  {base}: cutoff={cutoff}")
    ...
    for read in bam_in:
        ...
        if 'r1' in pair_buffer[read_name] and 'r2' in pair_buffer[read_name]:
            ...
            # 统计所有碱基个数并统一过滤
            # 只有当R1和R2在所有条件下都满足时才保留
            keep_pair = True
            
            for target_base, cutoff in zip(target_bases, cutoffs):
                r1_count = count_base_in_read(r1, target_base)
                r2_count = count_base_in_read(r2, target_base)
                
                # 如果R1或R2不满足当前条件，则不保留
                if r1_count >= cutoff or r2_count >= cutoff:
                    keep_pair = False
                    break
    ...
    # 对过滤后的BAM文件进行排序（建立索引需要BAM文件按坐标排序）
    logger.info(f"对过滤后的BAM文件进行排序...")
    sorted_filter_bam = filter_bam.parent / f"{filter_bam.stem}.sorted.bam"
    sort_cmd = f"samtools sort -o {sorted_filter_bam} {filter_bam}"
    run_cmd(sort_cmd, logger=logger)
    
    # 如果排序成功，替换原文件
    if sorted_filter_bam.exists():
        # 删除未排序的文件
        filter_bam.unlink()
        # 重命名排序后的文件
        sorted_filter_bam.rename(filter_bam)
        logger.info(f"BAM文件已排序: {filter_bam}")
    else:
        logger.warning(f"排序后的BAM文件未找到: {sorted_filter_bam}")
    
    # 为过滤后的BAM文件建立索引
    logger.info(f"为过滤后的BAM文件建立索引...")
    index_cmd = f"samtools index {filter_bam}"
    run_cmd(index_cmd, logger=logger)
    
    # 检查索引文件是否成功创建
    index_file = Path(str(filter_bam) + ".bai")
    if index_file.exists():
        logger.info(f"BAM索引文件已创建: {index_file}")
    else:
        logger.warning(f"BAM索引文件未找到: {index_file}")
    ...
    logger.info(f"  过滤BAM索引: {index_file}")
