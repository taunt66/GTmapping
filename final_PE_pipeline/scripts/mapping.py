...
def mapping_star(
    ...
    # 检查输入文件是否是gzip压缩格式
    # 对于.gz文件，STAR需要--readFilesCommand zcat参数
    r1_is_gz = str(r1_fastq).endswith('.gz')
    r2_is_gz = str(r2_fastq).endswith('.gz')
    
    if r1_is_gz != r2_is_gz:
        logger.warning(f"[STAR] R1 and R2 have different compression formats. R1: {r1_is_gz}, R2: {r2_is_gz}")
    ...
    if type == 'RNA':
        ...
        # 如果输入文件是gzip压缩格式，添加--readFilesCommand zcat
        if r1_is_gz or r2_is_gz:
            cmd_parts.insert(4, "--readFilesCommand zcat")  # 在--runThreadN之前插入
    else:  # type == 'DNA'
    ...
    logger.info(f"[STAR] Alignment completed. Output BAM (sorted by STAR): {output_bam}")

    # STAR 已经输出按坐标排序的 BAM，这里只需要建立索引
    sort_and_index_bam(output_bam, logger=logger, already_sorted=True, threads=threads)
...
def mapping_bowtie2(
    ...
    # 对 BAM 进行排序并建立索引（pileup 和后续分析需要排序+索引的 BAM）
    sort_and_index_bam(output_bam, logger=logger, already_sorted=False, threads=threads)
    
    logger.info(f"[Bowtie2] Alignment completed. Output BAM (sorted & indexed): {output_bam}")
    return output_bam
