...
def main_pipeline(
    ...
    # Step 0: 预处理FASTQ文件 - 转换并生成索引文件（根据type不同）
    logger.info("=" * 60)
    ...
    if dryrun:
        ...
        logger.info(f"[DRYRUN]   Type: {type}")
        logger.info(f"[DRYRUN]   Conversion rules: {', '.join(r1_convert_list)}")
        ...
        r1_converted, r1_idx = change_fastq(
            ...
            output_fastq=r1_converted_fq  # 明确指定输出文件路径
        )
        # 确保索引文件使用正确的文件名
    ...
    if dryrun:
        ...
        logger.info(f"[DRYRUN]   Type: {type}")
        logger.info(f"[DRYRUN]   Conversion rules: {', '.join(r2_convert_list)}")
        ...
        r2_converted, r2_idx = change_fastq(
            ...
            output_fastq=r2_converted_fq  # 明确指定输出文件路径
        )
        # 确保索引文件使用正确的文件名
    ...
    if dryrun:
        logger.info(f"[DRYRUN] Would restore original BAM and filter")
        logger.info(f"[DRYRUN]   Input BAM: {merged_bam}")
        logger.info(f"[DRYRUN]   Origin BAM: {origin_bam}")
        logger.info(f"[DRYRUN]   Filtered BAM: {filter_bam}")
        logger.info(f"[DRYRUN]   Filter conditions:")
        for base, cutoff in zip(filter_target_bases, filter_cutoffs):
            logger.info(f"[DRYRUN]     {base}: cutoff={cutoff}")
        final_bam = filter_bam
    else:
        logger.info(f"Restoring original BAM and filtering with {len(filter_target_bases)} conditions")
        logger.info(f"  Filter conditions:")
        for base, cutoff in zip(filter_target_bases, filter_cutoffs):
            logger.info(f"    {base}: cutoff={cutoff}")
        
        # 一次性恢复原始BAM并根据所有条件统一过滤
        origin_bam_result, filter_bam_result = filter_and_return_origin_bam(
            bam_path=merged_bam,
            r1_index_file=r1_index_file,
            r2_index_file=r2_index_file,
            origin_bam=origin_bam,
            filter_bam=filter_bam,
            target_bases=filter_target_bases,
            cutoffs=filter_cutoffs,
            logger=logger
        )
        final_bam = filter_bam_result
