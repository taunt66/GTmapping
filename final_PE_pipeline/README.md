...
### DNA流程
1. **文件准备**：碱基转换、构建索引（仅基因组）
2. **两次Mapping**：STAR基因组TC_GA（正链）、STAR基因组AG_CT（负链）
3. **BAM合并**：合并两个mapping结果
4. **原始BAM还原**：使用索引文件还原原始序列并过滤（T和G）
5. **Pileup分析**：按R2计数，生成A和C的覆盖度统计
6. **位点检测**：检测6mA和5mC修饰位点

### RNA流程
1. **文件准备**：提取最长转录本、碱基转换、构建索引
2. **三次Mapping**：STAR基因组AG_CT（正链）、STAR基因组TC_GA（负链）、bowtie2转录组AG_CT（正链）
3. **坐标转换**：转录组坐标转基因组坐标
4. **BAM合并**：合并三个mapping结果
5. **原始BAM还原**：使用索引文件还原原始序列并过滤（A和C）
6. **Pileup分析**：按R1计数，生成A和C的覆盖度统计
7. **位点检测**：检测m6A和m5C修饰位点
...
│   ├── pre_process.py           # FASTQ预处理（DNA/RNA特定）
